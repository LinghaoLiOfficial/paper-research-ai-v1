...
class AttentionVAE(nn.Module):
    ...
    def __init__(self, hidden_dim1, hidden_dim2, hidden_dim3, hidden_dim4):
        ...
        self.encoder_attention1 = nn.MultiheadAttention(embed_dim=hidden_dim1 - 1, num_heads=head, batch_first=True)

        self.encoder_fc1 = nn.Linear(hidden_dim1 - 1, hidden_dim2)
    ...
    def forward(self, x):
        ...
        h, _ = self.encoder_attention1(x, x, x)

        h = h.reshape(-1, x.shape[2])
        ...
        y = h.reshape(-1, x.shape[1], x.shape[2] + 1)
